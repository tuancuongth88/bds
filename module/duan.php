...
<?php 
	...
	$startRow = ($pageNum-1) * $pageSize;
     ...
     $totalRows = countRecord("tbl_rv_item","status=1 AND cate=2 $chuoit"); 
	 //echo "status=1 $chuoit AND parent in ({$parent}) order by $xapxep limit ";
	$bds=get_records("tbl_rv_item","status=1 $chuoit AND cate=2  order by $xapxep limit ".$startRow.",".$pageSize," "," "," ");	
...
<div class="f_cont clearfix">
    ...
    <article class="content">
        ...
        <div class="page">
                <div class="PageNum">
                    <?php  
                    echo pagesLinks_new_full_2013($totalRows, $pageSize , "http://www.giathinhcantho.com","p","page-du-an");
                    ?>
                </div>
                <div class="clear"></div>
            </div>
