<?php
	...
	if($_GET['act']=="linhtinh"){ // tim kiem
		//http://www.kenhcantho.com/ho-chi-minh/quan-1/phuong-ben-nghe/800-trieu-den-1-ti/
		$binhthuong=1;
		$field=array();
		$dem_para=0;
		for($i=1;$i<9;$i++){
...
<link rel="canonical" href="<?php print $root ?><?=$_SERVER['REQUEST_URI']?>" />
<?php if($dem_para<8){?>
<?php 
if (isset($_GET['pageNum'])==true) $pageNum = $_GET['pageNum'];
if ($pageNum<=0) $pageNum=1;
...
<meta property='og:url' content='http://<?php print $root ?><?=$_SERVER['REQUEST_URI']?>'/>
...
<?
	...
	elseif($_GET['act']=='product_detail' && $_GET['bds']){ // chi tiet tin - danh muc bds,da,dn - list bds theo thanh pho
		...
		if($row_rs_tit['id']!=""){// chi tiet tin
				//http://www.kenhcantho.com/athena-building-van-phong-cho-thue-quan-tan-binh.html
...
<meta property='og:title' content='<?php echo $title_t ;?>'/>
<meta property='og:description' content='<?php echo $description_t ;?>'/>
<meta property='og:url' content='<?= $root.$_SERVER['REQUEST_URI']?>'/>
<meta property='og:site_name' content='<?=$row_title_lap['copyright']?>'/>
...
<?				
		...
		if($row_rs_tit['id']==""){// danh muc
				$binhthuong=1;
				//http://www.kenhcantho.com/cho-thue-dat.html
				//http://www.kenhcantho.com/tin-tuc-bat-dong-san.html
...
<link rel="canonical" href="<?=$root.$_SERVER['REQUEST_URI']?>" />
<?php 
if (isset($_GET['pageNum'])==true) $pageNum = $_GET['pageNum'];
if ($pageNum<=0) $pageNum=1;
if($pageNum>1){
...
<link rel="canonical" href="http://kenhcantho.com<?=$_SERVER['REQUEST_URI']?>" />
<?php 
if (isset($_GET['pageNum'])==true) $pageNum = $_GET['pageNum'];
if ($pageNum<=0) $pageNum=1;
if($pageNum>1){
...
<meta property='og:title' content='<?php echo $title_t ;?>'/>
<meta property='og:description' content='<?php echo $description_t ;?>'/>
<meta property='og:url' content='http://kenhcantho.com<?=$_SERVER['REQUEST_URI']?>'/>
<meta property='og:site_name' content='<?=$row_title_lap['copyright']?>'/>
<meta property='og:type' content='website'/>
<?php if($row_rs_tit['image']=="") $hinh_tit="imgs/noimage_large.png";else $hinh_tit=$row_rs_tit['image'];?>
<?php if($row_rs_tit['image1']=="") $hinh_tit_k="imgs/noimage.png";else $hinh_tit_k=$row_rs_tit['image1'];?>
...
<?				
	...
	if($dem_ct==1 && $tpvaduong==0){
		...
		if($duong['id']>0 ){
			$binhthuong=1;
			// Thành Phố / Đường
			//http://www.kenhcantho.com/da-nang-duong-dang-thuy-tram.html

			$title_t =" Mua Bán Nhà Đất › ".$tp_tit['name']." › ".$duong['name'];
			$description_t="Mua Bán Nhà Đất ".$tp_tit['name'].", ".$duong['name'].", địa ốc, mua bán, rao vặt, mua nhà, bán nhà, cho thuê, cần thuê, nhà phố, chung cư, biệt thự, căn hộ, dự án, nhà xưởng.";		
		
			$keywords_t ="nha dat"." ".$tp_tit['name'].", ". "bat dong san"." ".$tp_tit['name'].", ". "dia oc"." ".$tp_tit['name'].", ". "mua nha"." ".$tp_tit['name'].", ".  " ban nha"." ".$tp_tit['name'].", ".  "cho thue nha"." ".$tp_tit['name'].", ". "can thue nha"." ".$tp_tit['name'].", "." nha pho"." ".$tp_tit['name'].", ". " chung cu"." ".$tp_tit['name'].", ". " biet thu"." ".$tp_tit['name'].", ". " can ho"." ".$tp_tit['name'].", ". " du an"." ".$tp_tit['name'].", ". "nha xuong"." ".$tp_tit['name'].", ".cat_kytu_dacbiet($duong['name'],1,1,0,0,1);
...
<?				
	...
	// Thành phố / Huyện
	//http://www.kenhcantho.com/binh-duong/thi-xa-di-an.html
...
<link rel="canonical" href="http://www.kenhcantho.com<?=$_SERVER['REQUEST_URI']?>" />
<?php 
if (isset($_GET['pageNum'])==true) $pageNum = $_GET['pageNum'];
if ($pageNum<=0) $pageNum=1;
if($pageNum>1){
...
<?				
	...
	//www.kenhcantho.com/ho-chi-minh/huyen-cu-chi/xa-phuoc-vinh-an.html
	elseif($_GET['act']=='phuong')
	{
		...
		$title_t    = " Mua Bán Nhà Đất › ".$tenquan." › ".$tenhuyen." › ".$tenphuong;
...
//Nhu Cầu
	elseif($_GET['act']=='nhucau'){
		$binhthuong=1;
		//http://www.kenhcantho.com/binh-duong/thi-xa-di-an/phuong-dong-hoa/nha-dat-ban.html
		$cate=$_GET['nhucau'];
		...
		$keywords_t ="".$catebds_kd." mat tien"." , ".$catebds_kd." pho" ." , ".$catebds_kd." biet thu" ." , ".$catebds_kd." villa" ." , ".$catebds_kd." xuong" ." , ".$catebds_kd." kho bai" ." , ".$catebds_kd." trong hem"  ." , ".$catebds_kd.", ".$tenhuyen_kd.", ".$tenquan_kd;
		if (isset($_GET['pageNum'])==true) $pageNum = $_GET['pageNum'];
		if ($pageNum<=0) $pageNum=1;
		if($pageNum>1) $themvao=" › Trang ".$pageNum;
		$title_t.=$themvao;
...
<? 
	...
	elseif($_GET['act']=='addbds'){
	    ...
	    $description_t="Chào mừng bạn đến với trang đăng tin bất động sản trên website <?php $root ?> bạn muốn bán nhà, cho thuê, sang nhượng cửa hàng, bạn hãy điền vào fom đăng tin dưới để thông tin của bạn được hiển thị trên website";
		$keywords_t   =$row_title_lap['keywords']; 
	} 
	elseif($_GET['act']=='editbds'){
		$title_t      ="Sửa tin đăng bất động sản";
	    $description_t="Chào mừng bạn đến với trang sửa tin đăng bất động sản trên website <?php $root ?> bạn những thông tin còn thiếu hay chưa đúng bạn hãy sửa trực tiếp trên fom và nhấn chấp nhận";
		$keywords_t   =$row_title_lap['keywords']; 
	} 
...
<meta property='og:image' content='<?=$linkrootbds?><?=$row_rs_tit['image']?>'/>
	...
	elseif($_GET['act']=='dangky'){
	    ...
	    $description_t="Chào mừng bạn đến với trang đăng ký tài khoản trên website <?php $root ?> để được là thành viên chính thức của chúng tôi bạn hãy nhập thông tin đăng ký để là thành viên chính thức";
		$keywords_t =$row_title_lap['keywords'];	
	}
	elseif($_GET['act']=='dangnhap'){
		$title_t    ="Đăng nhập tài khoản trên website <?php $root ?>";
	    $description_t="Chào mừng bạn đến với trang đăng nhập tài khoản trên website <?php $root ?> bạn vui lòng điền thông tin đăng nhập để sử dụng chức năng quảng cáo đăng tin bất động sản trên website ";
		$keywords_t =$row_title_lap['keywords'];	
	}
