...
// tu khoa tach ra khoang 3 tu la duoc
     $totalRows = countRecord("tbl_rv_item","status=1 $str_tag  and cate=2 and (  name LIKE '%$tag%' or  detail LIKE '%$tag%' or  keyword LIKE '%$tag%')"); 
	// echo "status=1 and (  name LIKE '%$tag%' or  detail LIKE '%$tag%' or  keyword LIKE '%$tag%')  order by $xapxep limit ".$startRow.",".$pageSize;
	$bds=get_records("tbl_rv_item","status=1 $str_tag  and cate=2 and (  name LIKE '%$tag%' or  detail LIKE '%$tag%' or  keyword LIKE '%$tag%')  order by $xapxep limit ".$startRow.",".$pageSize," "," "," ");	
