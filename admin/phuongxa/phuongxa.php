...
if(isset($frame)==true){
	if($_SESSION['kt_login_level']!=4 && $_SESSION['kt_login_level']!=-1){
		...
		if(userPermissEdit($row_tbl_users['listEdit'],15,1)!=true) header("location: admin.php");
	}
}else{
...
$(document).ready(function() {	  
	...
	<?php 
if(userPermissEdit($row_tbl_users['listEdit'],15,7 )==true  ||  $_SESSION['kt_login_level']==4 ||  $_SESSION['kt_login_level']==-1){
?>
	...
	obj = this;
		...
		$.ajax({
		   ...
		   data: 'id='+ id +'&table=tbl_phuongxa' +'&type=1&cate=15',
...
<?php 
if(userPermissEdit($row_tbl_users['listEdit'],15,6)==true ||  $_SESSION['kt_login_level']==4 ||  $_SESSION['kt_login_level']==-1 ){
?>
	...
	obj = this;
		...
		$.ajax({
		   ...
		   data: 'id='+ id +'&table=tbl_phuongxa' +'&type=2&cate=15',
    ...
    <div class="span12">
        <div class="box-widget">
            ...
            <div class="widget-container">
				...
				<?
				 if(userPermissEdit($row_tbl_users['listEdit'],15,4)==true ||  $_SESSION['kt_login_level']==4  ||   $_SESSION['kt_login_level']==-1  ){
					 
					switch ($_GET['action']){
						case 'del' :
							$id = $_GET['id'];
							$r = getRecord("tbl_phuongxa","id=".$id);
							$resultParent = mysql_query("select id from tbl_phuongxa where parent='".$id."'",$conn);
							if (mysql_num_rows($resultParent) <= 0){
								@$result = mysql_query("delete from tbl_phuongxa where id='".$id."'",$conn);
								if ($result){
                ...
                if($parent!=-1 || $parent1!=-1) {
						...
						else $parenstrt=getParent("tbl_quanhuyen",$parent);
						 $where="1=1   and (id='{$tukhoa}' or name LIKE '%$tukhoa%' or '{$tukhoa}'=-1) and  ( parent in ({$parenstrt}) or id=$parent)";
						//$where="1=1   and (id='{$tukhoa}' or name LIKE '%$tukhoa%' or '{$tukhoa}'=-1) and  ( parent=$parent)";
					}
                ...
                $MAXPAGE=1;
				$totalRows=countRecord("tbl_phuongxa",$where);
				
                if ($_REQUEST['cat']!='') $where="parent=".$_REQUEST['cat']; ?>
                <form method="POST" action="#" name="frmForm" enctype="multipart/form-data">
                <input type="hidden" name="page" value="<?=$page?>">
                <input type="hidden" name="act" value="phuongxa">
                <?
               // $pageindex = createPage(countRecord("tbl_phuongxa",$where),"./?act=quanhuyen&cat=".$_REQUEST['cat']."&page=",$MAXPAGE,$page)?>
                ...
                <? if ($_REQUEST['code']==1) $errMsg = 'Cập nhật thành công.'?>
                    ...
                    <table width="100%"  class="admin_table">
                            ...
                            <thead>
                                ...
                                <tr align="center" >
                                  <td valign="middle" style="background-color:#F0F0F0; height:40px; padding-left:20px" colspan="10">  
                                  		<? //comboCategory('ddCat',getArrayCategory('tbl_phuongxa'),'list_tim_loc',$parent,1)?>
                                        ...
                                        <select name="ddCat" id="ddCat" class="list_tim_loc"> 
                                            ...
                                            <?php   
											$gt=get_records("tbl_quanhuyen_category"," status=1 ","id DESC"," "," ");
                                ...
                                <tr >
                                  <td valign="middle" align="left" style="background-color:#F0F0F0; height:40px; padding-left:20px" colspan="10"> 
                                        <div class="link_loc" style="width:80px;text-align:center; padding:3px; border:solid 1px #999; float:left; margin-right:5px;<?php if($noibat==0  &&  $anhien==0) echo 'background-color:#FF0; color:#000;';else echo 'background-color:#FFF; color:#FFF;"';?>">
                                        	<a href="admin.php?act=quanhuyen&tang=1&anhien=-1&noibat=-1">Tất cả</a>
                                        </div>
                                        <div class="link_loc" style="width:80px; text-align:center; padding:3px; border:solid 1px #999; float:left; margin-right:5px;<?php if($tang==1) echo 'background-color:#FF0; color:#000;';else echo 'background-color:#FFF; color:#FFF;"';?>" >
                                        	<a href="admin.php?act=quanhuyen&tang=1&anhien=<?php echo $_SESSION['kt_anhien'] ?>&noibat=<?php echo $_SESSION['kt_noibat']?>">Tăng dần</a>
                                        </div>
                                        <div class="link_loc" style="width:80px;text-align:center; padding:3px; border:solid 1px #999; float:left; margin-right:5px;<?php if($tang==0) echo 'background-color:#FF0; color:#000;';else echo 'background-color:#FFF; color:#FFF;"';?>">
                                        	<a href="admin.php?act=quanhuyen&tang=0&anhien=<?php echo $_SESSION['kt_anhien'] ?>&noibat=<?php echo $_SESSION['kt_noibat']?>">Giảm dần</a>
                                        </div>
                                        <div class="link_loc" style="width:80px;text-align:center; padding:3px; border:solid 1px #999; float:left; margin-right:5px;<?php if($anhien==1) echo 'background-color:#FF0; color:#000;';else echo 'background-color:#FFF; color:#FFF;"';?>">
                                        	<a href="admin.php?act=quanhuyen&tang=<?php echo $_SESSION['kt_tang'] ?>&anhien=1&noibat=<?php echo $_SESSION['kt_noibat'] ?>"> Ẩn </a>
                                        </div>
                                         <div class="link_loc" style="width:80px;text-align:center; padding:3px; border:solid 1px #999; float:left; margin-right:5px;<?php if($anhien==0) echo 'background-color:#FF0; color:#000;';else echo 'background-color:#FFF; color:#FFF;"';?>">
                                        	<a href="admin.php?act=quanhuyen&tang=<?php echo $_SESSION['kt_tang'] ?>&anhien=0&&noibat=<?php echo $_SESSION['kt_noibat'] ?>">Hiện</a>
                                        </div>
                                        <div class="link_loc" style="width:80px;text-align:center; padding:3px; border:solid 1px #999; float:left; margin-right:5px;<?php if($noibat==1) echo 'background-color:#FF0; color:#000;';else echo 'background-color:#FFF; color:#FFF;"';?>">
                                        	<a href="admin.php?act=quanhuyen&tang=<?php echo $_SESSION['kt_tang'] ?>&anhien=<?php echo $_SESSION['kt_anhien'] ?>&noibat=1">Nổi bật</a>
                                        </div>
                                         <div class="link_loc" style="width:80px;text-align:center; padding:3px; border:solid 1px #999; float:left; margin-right:5px;<?php if($noibat==0) echo 'background-color:#FF0; color:#000;';else echo 'background-color:#FFF; color:#FFF;"';?>">
                                        	<a href="admin.php?act=quanhuyen&tang=<?php echo $_SESSION['kt_tang'] ?>&anhien=<?php echo $_SESSION['kt_anhien'] ?>&noibat=0">ko nổi bật</a>
                                        </div>
                                  </td>
                                </tr>
                               <tr>
                                  ...
                                  <td align="center" class="PageNum" colspan="6" height="30">
                                    	<?php echo pagesListLimit($totalRows,$pageSize);?>   
                                  </td>
                                  
                                 <td width="81" align="center" colspan="1">
                                 <? if(userPermissEdit($row_tbl_users['listEdit'],15,2)==true ||  $_SESSION['kt_login_level']==4 ||   $_SESSION['kt_login_level']==-1 ){?>
                                  <div><a href="admin.php?act=phuongxa_m">
					    ...
					    $sql="select *,DATE_FORMAT(date_added,'%d/%m/%Y %h:%i') as dateAdd,DATE_FORMAT(last_modified,'%d/%m/%Y %h:%i') as dateModify from tbl_phuongxa where parent<>0 and $where $sortby   limit ".($startRow).",".$pageSize;
						$result=mysql_query($sql,$conn);
						$i=0;
						while($row=mysql_fetch_array($result)){
						$parent = getRecord('tbl_quanhuyen','id = '.$row['parent']);
						...
						?>
                            <tr>
                                ...
                                <td align="left">
                                    Quận/ Huyện: <?=$parent['name']?> <br />
                                    Tỉnh/ Thành: <?=get_field('tbl_quanhuyen_category','id',$parent['parent'],'name') ?> 
                                </td>
                                <td align="center">
                                    <?=$row['sort']?>
                                </td>
                                ...
                                <td align="center">
                                <? if(userPermissEdit($row_tbl_users['listEdit'],15,3)==true ||  $_SESSION['kt_login_level']==4 ||   $_SESSION['kt_login_level']==-1 ){?>
                                    <a href="admin.php?act=phuongxa_m&cat=<?=$_REQUEST['cat']?>&page=<?=$_REQUEST['page']?>&id=<?=$row['id']?>"><img src="images/icon3.png"/></a>
                                <? }?>
                               	<? if(userPermissEdit($row_tbl_users['listEdit'],15,4)==true ||  $_SESSION['kt_login_level']==4 ||   $_SESSION['kt_login_level']==-1 ){?>
                                    <a  title="Xóa" href="admin.php?act=phuongxa&action=del&page=<?=$_REQUEST['page']?>&id=<?=$row['id']?>" onclick="return confirm('Bạn có muốn xoá luôn không ?');" ><img src="images/icon4.png" width="20" border="0" /></a>
                                <?php }?>
                                </td>
                            </tr>
                         <?php }?> 
