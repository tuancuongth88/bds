...
if(isset($frame)==true){
	if($_SESSION['kt_login_level']!=4 && $_SESSION['kt_login_level']!=-1){
		...
		if( isset($_GET['id']) || !empty($_POST['id'])) {if(userPermissEdit($row_tbl_users['listEdit'],15,3)!=true) header("location: admin.php");}
		elseif(userPermissEdit($row_tbl_users['listEdit'],15,2)!=true) header("location: admin.php");
	}
}else{
...
if (isset($_POST['btnSave'])){
	...
	if ($errMsg==''){
		if (!empty($_POST['id'])){
			$oldid = $_POST['id'];
			$sql = "update tbl_phuongxa set name='".$name."' , parent='".$parent1."' , parent1='".$parent."' , sort='".$sort."' , last_modified=now()  where id='".$oldid."'";
		}else{
			$sql = "insert into tbl_phuongxa (  name ,  sort, parent , parent1, status,  date_added, last_modified ) values ('" .$name."','".$sort."','".$parent1."','".$parent."','1',now(),now())";
		}
		if (mysql_query($sql,$conn)){
			if(empty($_POST['id'])) $oldid = mysql_insert_id();
			$r = getRecord("tbl_phuongxa","id=".$oldid);

			$arrField = array(
			"subject"          => "'".cat_kytu_dacbiet($name,1,1,0,1,1)."'"
			);// ko them id vao cuoi cho dep 
			$result = update("tbl_phuongxa",$arrField,"id=".$oldid);
			...
			if($sqlUpdateField!='')	{
				$sqlUpdate = "update tbl_phuongxa set $sqlUpdateField where id='".$oldid."'";
	...
	if ($errMsg == '')
		echo '<script>window.location="admin.php?act=phuongxa&cat='.$_REQUEST['cat'].'&page='.$_REQUEST['page'].'&code=1"</script>';
}else{
	if (isset($_GET['id'])){
		$oldid=$_GET['id'];
		$page = $_GET['page'];
		$sql = "select * from tbl_phuongxa where id='".$oldid."'";
        ...
        <div class="box-widget">
            ...
            <div class="widget-container">
                <div class="widget-block">
                   ...
                   <form method="post" name="frmForm" enctype="multipart/form-data" action="admin.php?act=quanhuyen_m">
                        ...
                        <input type="hidden" name="act" value="phuongxa_m">
                        ...
                        <input type="hidden" name="page" value="<?=$_REQUEST['page']?>">
                            ...
                            <table  class="table_chinh">
                                ...
                                <tr>
                                  <td class="table_chu_tieude_them" colspan="2" align="center" valign="middle"  >PHƯỜNG XÃ</td>
                              </tr>
                                <tr>
