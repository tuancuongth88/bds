...
if(isset($frame)==true){
	if($_SESSION['kt_login_level']!=4 && $_SESSION['kt_login_level']!=-1){
		...
		if(userPermissEdit($row_tbl_users['listEdit'],18,1)!=true) header("location: admin.php");
	}
}else{
        ...
        <div class="box-widget">
            ...
            <div class="widget-container">
                <div class="widget-block">
						...
						<?
							if(userPermissEdit($row_tbl_users['listEdit'],18,4)==true ||  $_SESSION['kt_login_level']==4  ||   $_SESSION['kt_login_level']==-1  ){
							...
							$where="1=1 and (id='{$tukhoa}' or mobile LIKE '%$tukhoa%' or username LIKE '%$tukhoa%' or name LIKE '%$tukhoa%' or '{$tukhoa}'=-1) and  ( idgroup=3 or   idgroup=4 )";
                                ...
                                <tr >
                                  <td valign="middle" align="left" style="background-color:#F0F0F0; height:40px; padding-left:20px" colspan="10"> 
                                        <div class="link_loc" style="width:80px;text-align:center; padding:3px; border:solid 1px #999; float:left; margin-right:5px;<?php if($noibat==0  &&  $anhien==0) echo 'background-color:#FF0; color:#000;';else echo 'background-color:#FFF; color:#FFF;"';?>">
                                        	<a href="admin.php?act=user&tang=1&anhien=-1&noibat=-1">Tất cả</a>
                                        </div>
                                        <div class="link_loc" style="width:80px; text-align:center; padding:3px; border:solid 1px #999; float:left; margin-right:5px;<?php if($tang==1) echo 'background-color:#FF0; color:#000;';else echo 'background-color:#FFF; color:#FFF;"';?>" >
                                        	<a href="admin.php?act=user&tang=1&anhien=<?php echo $_SESSION['kt_anhien'] ?>&noibat=<?php echo $_SESSION['kt_noibat']?>">Tăng dần</a>
                                        </div>
                                        <div class="link_loc" style="width:80px;text-align:center; padding:3px; border:solid 1px #999; float:left; margin-right:5px;<?php if($tang==0) echo 'background-color:#FF0; color:#000;';else echo 'background-color:#FFF; color:#FFF;"';?>">
                                        	<a href="admin.php?act=user&tang=0&anhien=<?php echo $_SESSION['kt_anhien'] ?>&noibat=<?php echo $_SESSION['kt_noibat']?>">Giảm dần</a>
                                        </div>
                                        <div class="link_loc" style="width:80px;text-align:center; padding:3px; border:solid 1px #999; float:left; margin-right:5px;<?php if($anhien==1) echo 'background-color:#FF0; color:#000;';else echo 'background-color:#FFF; color:#FFF;"';?>">
                                        	<a href="admin.php?act=user&tang=<?php echo $_SESSION['kt_tang'] ?>&anhien=1&noibat=<?php echo $_SESSION['kt_noibat'] ?>"> Ẩn </a>
                                        </div>
                                         <div class="link_loc" style="width:80px;text-align:center; padding:3px; border:solid 1px #999; float:left; margin-right:5px;<?php if($anhien==0) echo 'background-color:#FF0; color:#000;';else echo 'background-color:#FFF; color:#FFF;"';?>">
                                        	<a href="admin.php?act=user&tang=<?php echo $_SESSION['kt_tang'] ?>&anhien=0&&noibat=<?php echo $_SESSION['kt_noibat'] ?>">Hiện</a>
                                        </div>
                                        <div class="link_loc" style="width:80px;text-align:center; padding:3px; border:solid 1px #999; float:left; margin-right:5px;<?php if($noibat==1) echo 'background-color:#FF0; color:#000;';else echo 'background-color:#FFF; color:#FFF;"';?>">
                                        	<a href="admin.php?act=user&tang=<?php echo $_SESSION['kt_tang'] ?>&anhien=<?php echo $_SESSION['kt_anhien'] ?>&noibat=1">Nổi bật</a>
                                        </div>
                                         <div class="link_loc" style="width:80px;text-align:center; padding:3px; border:solid 1px #999; float:left; margin-right:5px;<?php if($noibat==0) echo 'background-color:#FF0; color:#000;';else echo 'background-color:#FFF; color:#FFF;"';?>">
                                        	<a href="admin.php?act=user&tang=<?php echo $_SESSION['kt_tang'] ?>&anhien=<?php echo $_SESSION['kt_anhien'] ?>&noibat=0">ko nổi bật</a>
                                        </div>
                                  </td>
                                </tr>
                               <tr>
                                 ...
                                 <? if(userPermissEdit($row_tbl_users['listEdit'],18,2)==true ||  $_SESSION['kt_login_level']==4 ||   $_SESSION['kt_login_level']==-1 ){?>
                                  <div><a href="admin.php?act=user_m">
                                ...
                                <tr class="admin_tieude_table">
											...
											<td width="10%" align="center"><span class="title"><a class="title" href="<?=getLinkSort(3)?>">Hình</a></span></td>
											<td width="7%" align="center"><a class="title" href="<?=getLinkSort(4)?>">Username</a></td>
											...
											<td width="12%" align="center"><a class="title" href="<?=getLinkSort(12)?>">Ngày tạo</a></td>
									...
									$sql="select *,DATE_FORMAT(date_added,'%d/%m/%Y %h:%i') as dateAdd,DATE_FORMAT(last_modified,'%d/%m/%Y %h:%i') as dateModify from tbl_users where  $where $sortby $direction limit ".($startRow).",".$pageSize;
									...
									?>
										<tr>
											...
											<td align="left">
                                            <p>
											<a href="?act=user_m&id=<?php echo $row['id'];?>">
												<?php
													if($row['idgroup']==1){echo 'Customer';}
													elseif($row['idgroup']==2){echo 'User';}
													elseif($row['idgroup']==3){echo 'Mode';}
													elseif($row['idgroup']==4){echo 'Admin';}
												  ?>
											</a>
                                            </p> 
                                            <p>
                                            <a href="?act=user_permiss&id=<?php echo $row['id'];?>">
                                             ...
                                             <p>
                                            <a href="?act=user_permiss_city&id=<?php echo $row['id'];?>">
												 Tỉnh
											</a>
                                             </p>
											</td>
											...
											<td align="left">
												<p>Ngày tạo: <?=$row['dateAdd']?> </p>
                                                <p>Ngày sửa: <?=$row['dateModify']?> </p>
											</td>
