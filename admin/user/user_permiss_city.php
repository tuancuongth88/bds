...
if (isset($_POST['them'])==true)//isset kiem tra submit
	{
		...
		{	
			settype($idgroup,"int");
			settype($id,"int");
			$sql = "UPDATE tbl_users SET listCity='{$ddk}' WHERE id='{$id}'";
    ...
    <div class="span12">
        <div class="box-widget">
            <div class="widget-head overview-chart clearfix">
                <span class="h-icon"><i class="gray-icons graph"></i></span> <h4 class="pull-left">Quyền thành viên</h4>
            ...
            <center>   
                ...
                <hr />
                <?php
                $lap_quyen=get_records('tbl_quanhuyen_category','status=1','sort ASC',' ',' ');
                while ($row_lap_quyen=mysql_fetch_assoc($lap_quyen)){?>  
                
                <input <?php if(number_in_list($row_tbl_users['listCity'],$row_lap_quyen['id'])){ ?> checked="checked" <?php } ?> name="cauhinh[]" class="check_md" type="checkbox" value="<?php echo $row_lap_quyen['id'] ?>" />
