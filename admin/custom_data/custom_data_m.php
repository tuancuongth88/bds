...
if(isset($frame)==true){
	if($_SESSION['kt_login_level']!=4 && $_SESSION['kt_login_level']!=-1){
		...
		if( isset($_GET['id']) || !empty($_POST['id'])) {if(userPermissEdit($row_tbl_users['listEdit'],20,3)!=true) header("location: admin.php");}
		elseif(userPermissEdit($row_tbl_users['listEdit'],20,2)!=true) header("location: admin.php");
	}
}else{
...
if (isset($_POST['btnSave'])){
	...
	$idgroup       = isset($_POST['idgroup']) ? $_POST['idgroup'] : 1;
	 ...
	 if(userPermissEdit($row_tbl_users['listEdit'],20,5)==true ||  $_SESSION['kt_login_level']==4 ||   $_SESSION['kt_login_level']==-1 )   echo "";
	 else $idgroup=1;
	...
	if ($errMsg==''){
		if (!empty($_POST['id'])){
			...
			if(userPermissEdit($row_tbl_users['listEdit'],20,5)==true ||  $_SESSION['kt_login_level']==4 || $_SESSION['kt_login_level']==-1 ){
				if($repassword=="") $sql = "update tbl_users set name='".$name."', username='".$username."', mobile='".$mobile."',email='".$email."',address='".$address."',cmnd='".$cmnd."',ghichu='".$ghichu."',sex='".$gioitinh."',status='".$status."',idgroup='".$idgroup."',last_modified=now() where id='".$oldid."'";
				else $sql = "update tbl_users set name='".$name."', username='".$username."', password='".$password."', mobile='".$mobile."',email='".$email."',address='".$address."',cmnd='".$cmnd."',ghichu='".$ghichu."',sex='".$gioitinh."',status='".$status."',idgroup='".$idgroup."',last_modified=now() where id='".$oldid."'";
				 
			}else{
				if($repassword=="") $sql = "update tbl_users set name='".$name."', username='".$username."', mobile='".$mobile."',email='".$email."',address='".$address."',cmnd='".$cmnd."',ghichu='".$ghichu."',sex='".$gioitinh."',status='".$status."',last_modified=now() where id='".$oldid."'";
	...
	if ($errMsg == '') {
		 echo '<script>window.location="admin.php?act=custom_data&cat='.$_REQUEST['cat'].'&page='.$_REQUEST['page'].'&code=1"</script>'  ;
	}
}else{
	if (isset($_GET['id'])){
		$oldid=$_GET['id'];
        ...
        <div class="box-widget">
          <div class="widget-container">
              <div class="widget-block">
                   ...
                   <form method="post" name="frmForm" enctype="multipart/form-data" action="admin.php?act=custom_data_m">
                        <input type="hidden" name="act" value="custom_data_m">
