        ...
        <div class="box-widget">
            ...
            <div class="widget-container">
                <div class="widget-block">
                    ...
                    $where="1=1 AND cate=0  AND (id='{$tukhoa}' or name LIKE '%$tukhoa%' or '{$tukhoa}'=-1)";
					
					$where.=" $strDate AND type=1 AND ( iduser='{$idu}' or '{$idu}'=-1) "; 
                    ...
                    <form method="POST" action="admin.php?act=bds_data" name="frmForm" enctype="multipart/form-data">
    
                    <input type="hidden" name="page" value="<?=$page?>">
    
                    <input type="hidden" name="act" value="bds_data">
                            ...
                            <thead>
                                ...
                                <tr align="center" >
                                  <td valign="middle"  colspan="7">
                                    <center>
                                        <div class="table_chu_tieude"><strong>Thống kê tin rao</strong></div>
                                    </center>
                                   </td>
                                </tr>
                                <tr align="center" >
...
<script src="http://code.jquery.com/ui/1.10.3/jquery-ui.js"></script>
                                    ...
                                    <input name="datebegin" id="datebegin" type="text" class="table_khungnho"  style="width:100px;" value="<? echo $datebeginold?>" placeholder="Ngày bắt đầu "/>
                                    <input name="dateend" id="dateend" type="text" class="table_khungnho"  style="width:100px;" value="<? echo $dateendold?>" placeholder="Ngày kết thúc " />
                                    <select name="user" id="user" class="table_khungnho"> 
                                            ...
                                            <option value="<?php echo $row['id']; ?>" data-alternative-spellings="<?php echo $row['name']; ?>" <?php if($user==$row['username']) echo 'selected="selected"';?>  ><?php echo $row['username']; ?></option> 
                                            <?php } ?>
                                ...
                                <tr >
                                  ...
                                  Tin nhập liệu: <?=countRecord("tbl_rv_item","type=1  and cate=0 $strDate AND ( iduser='{$idu}' or '{$idu}'=-1)")?> tin -  Tin đã chuyệt: <?=countRecord("tbl_rv_item","type=1 and cate=0 and active=1 $strDate AND ( iduser='{$idu}' or '{$idu}'=-1)")?> tin
                                  
                                  - Thành tiền: <?=number_format(countRecord("tbl_rv_item","type=1 and cate=0 and active=1 $strDate AND ( iduser='{$idu}' or '{$idu}'=-1)")*$priceNL,0);?> đ</td>
                                </tr>
                               <tr>
                           ...
                           echo  $sql="select *,DATE_FORMAT(date_added,'%d/%m/%Y %h:%i') as dateAdd,DATE_FORMAT(last_modified,'%d/%m/%Y %h:%i') as dateModify,DATE_FORMAT(date_actived,'%d/%m/%Y %h:%i') as dateActive from tbl_rv_item where  $where $sortby  limit ".($startRow).",".$pageSize;
                            ...
                            ?>
                                ...
                                <tr>
                                    ...
                                    <td align="center"><?=$row['dateActive']?></td>
                                    <td align="center">
