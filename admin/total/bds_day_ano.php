...
{  
	...
	if($_POST['month']!=NULL){$month=$_POST['month'];}else {$month="";}
	$_SESSION['kt_month']=$month;
	
	if($_POST['year']!=NULL){$year=$_POST['year'];}else {$year="";}
	$_SESSION['kt_year']=$year;	
	
	if($_POST['day']!=NULL){$day=$_POST['day'];}else {$day="";}
	$_SESSION['kt_day']=$day;	
}
        ...
        <div class="box-widget">
            ...
            <div class="widget-container">
                <div class="widget-block">
					...
					if($month !="" && $year!="" && $day!="") {
						$where.=" AND type=1 AND ( iduser='{$idu}' or '{$idu}'=-1)  and ( Year(date_actived) = $year  AND Month(date_actived) = $month  AND DAY(date_actived) = $day )"; 
					}elseif($month !="" && $year!=""){
						$where.=" AND type=1 AND ( iduser='{$idu}' or '{$idu}'=-1)  and ( Year(date_actived) = $year  AND Month(date_actived) = $month )"; 
					}elseif($year!=""){
						$where.=" AND type=1 AND ( iduser='{$idu}' or '{$idu}'=-1)  and ( Year(date_actived) = $year)"; 
					}else  $where.=" and 1 >2";
                    ...
                    <form method="POST" action="admin.php?act=bds_day_ano" name="frmForm" enctype="multipart/form-data">
    
                    <input type="hidden" name="page" value="<?=$page?>">
    
                    <input type="hidden" name="act" value="bds_day_ano">
                            ...
                            <thead>
                                ...
                                <tr align="center" >
                                  <td valign="middle"  colspan="7">
                                    <center>
                                        <div class="table_chu_tieude"><strong>Tin theo ngày</strong></div>
                                    </center>
                                   </td>
                                </tr>
                                <tr align="center" >
...
<script src="http://code.jquery.com/ui/1.10.3/jquery-ui.js"></script>
									...
									<select name="day" id="day" class="table_khungnho">
									  <option value=""> Chọn ngày</option>
									  <? for($i=1;$i<32;$i++) {?>
									  <option value="<?=$i?>" <? if($day==$i) echo 'selected="selected"';?>>
									    <?="Ngày ".$i?>
								      </option>
									  <? } ?>
								    </select>
									<select name="month" id="month" class="table_khungnho">
					 					<? for($i=1;$i<13;$i++) {?>
                                   	  <option value="<?=$i?>" <? if($month==$i) echo 'selected="selected"';?>> <?="Tháng ".$i?></option>
                                        <? } ?>
								    </select>
                                    <select name="year" id="year" class="table_khungnho">
					 					<? for($i=2013;$i<2020;$i++) {?>
                                        	<option value="<?=$i?>" <? if($year==$i) echo 'selected="selected"';?>> <?="Năm ".$i?></option>
                                        <? } ?>
								    </select>
                                ...
                                <tr >
                                  <td valign="middle" align="left" style="background-color:#F0F0F0; height:40px; padding-left:20px" colspan="7">
                                   Tổng số tin: <?=$totalRows?> tin - Thành tiền: <?=number_format($totalRows*$priceNL,0);?> đ
                                  </td>
                                </tr>
                               <tr>
