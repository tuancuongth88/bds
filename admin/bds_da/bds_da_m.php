...
if(isset($frame)==true){
	if($_SESSION['kt_login_level']!=4 && $_SESSION['kt_login_level']!=-1){
		...
		if( isset($_GET['id']) || !empty($_POST['id'])) {if(userPermissEdit($row_tbl_users['listEdit'],6,3)!=true) header("location: admin.php");}
		elseif(userPermissEdit($row_tbl_users['listEdit'],6,2)!=true) header("location: admin.php");
	}
}else{
...
<div class="f_dt">
		<div class="table_chu_tieude">
        	SỬA THÔNG TIN DỰ ÁN BẤT ĐỘNG SẢN
        </div>
        <div class="clear"> </div>
    	<form action="admin.php?act=bds_da_m" method="post" enctype="multipart/form-data" name=formdk id=formdk>
        <input type="hidden" name="act" value="bds_da_m" />
            ...
            $(document).ready(function() {
                ...
                $(".upload").click(function() {
                    ...
                    //          break;
                            ...
                            $.ajax({
                               type: "POST",
                               url:'<?php echo $linkrootbds?>admin/file.php?id='+id+"&loai=_da",
                               processData: false,
                               contentType: false,
                               data: fd,
                               cache: false,
