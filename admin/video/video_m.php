...
if(isset($frame)==true){
	 if($_SESSION['kt_login_level']!=4 && $_SESSION['kt_login_level']!=-1){
		...
		if( isset($_GET['id']) || !empty($_POST['id'])) {if(userPermissEdit($row_tbl_users['listEdit'],11,3)!=true) header("location: admin.php");}
		elseif(userPermissEdit($row_tbl_users['listEdit'],11,2)!=true) header("location: admin.php");
	} 
}else{
...
if (isset($_POST['btnSave'])){
	...
	if ($errMsg==''){
		if (!empty($_POST['id'])){
			$oldid = $_POST['id'];
			  $sql = "update tbl_rv_video set idshop='".$idshop."',detail_short='".$detail_short."',detail='".$detail."',name='".$name."', link='".$link."', title ='".$title ."', description ='".$description ."', keyword='".$keyword."',  last_modified=now() where id='".$oldid."'";
		}else{
			  $sql = "insert into tbl_rv_video ( idshop , name, detail_short , detail , link ,   title, description , keyword , status , date_added, last_modified ) values ('".$idshop."','".$name."','".$detail_short."','".$detail."','".$link."','".$title."','".$description."','".$keyword."','1',now(),now()  )";
		}
