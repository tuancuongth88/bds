...
if(isset($frame)==true){
	if($_SESSION['kt_login_level']!=4 && $_SESSION['kt_login_level']!=-1){
		...
		if( isset($_GET['id']) || !empty($_POST['id'])) {if(userPermissEdit($row_tbl_users['listEdit'],3,3)!=true) header("location: admin.php");}
		elseif(userPermissEdit($row_tbl_users['listEdit'],3,2)!=true) header("location: admin.php");
	}
}else{
...
$path = "../web/images/bdsnew";
$pathdb = "images/bdsnew";
if (isset($_POST['btnSave'])){
	...
	if ($errMsg==''){
		if (!empty($_POST['id'])){
			...
			$sql = "insert into tbl_rv_category (code, cate , name, parent, subject, detail_short, detail, title , description , keyword , sort, status,  date_added, last_modified, lang) values ('".$code."','1','".$name."','".$parent1."','".$subject."','".$detail_short."','".$detail."','".$title."','".$description."','".$keyword."','".$sort."','1',now(),now(),'".$lang."')";
		}
	...
	if ($errMsg == '')
		echo '<script>window.location="admin.php?act=bds_news_category&cat='.$_REQUEST['cat'].'&page='.$_REQUEST['page'].'&code=1"</script>';
}else{
	if (isset($_GET['id'])){
		$oldid=$_GET['id'];
		$page = $_GET['page'];
            ...
            <div class="widget-container">
                <div class="widget-block">
                   ...
                   <form method="post" name="frmForm" enctype="multipart/form-data" action="admin.php?act=bds_category">
                        ...
                        <input type="hidden" name="act" value="bds_news_category_m">
                            ...
                            <table  class="table_chinh">
                                ...
                                <tr>
                                  ...
                                  <td valign="middle"><select name="ddCat" id="ddCat" class="table_list">
                                    ...
                                    <?php   
                                    $gt=get_records("tbl_rv_category","parent=2 and cate=1 and status=0 ","id DESC"," "," ");
