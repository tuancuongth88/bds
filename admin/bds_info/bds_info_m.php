...
if(isset($frame)==true){
	if($_SESSION['kt_login_level']!=4 && $_SESSION['kt_login_level']!=-1){
		...
		if( isset($_GET['id']) || !empty($_POST['id'])) {if(userPermissEdit($row_tbl_users['listEdit'],9,3)!=true) header("location: admin.php");}
		elseif(userPermissEdit($row_tbl_users['listEdit'],9,2)!=true) header("location: admin.php");
	}
}else{
...
$path = "../images/bds_info";
$pathdb = "images/bds_info";
if (isset($_POST['btnSave'])){
	...
	$txtDetailShort  = isset($_POST['txtDetailShort']) ? trim($_POST['txtDetailShort']) : '';
...
/*	if ($parent=="") $errMsg .= "Hãy chọn ít nhất một chuyên mục!<br>";*/
	...
	if ($errMsg==''){
		if (!empty($_POST['id'])){
			$oldid = $_POST['id'];
			   $sql = "update tbl_rv_item set   name='".$name."',khachhang='".$khachhang."',diachi='".$diachi."',email='".$email."',dienthoai='".$dienthoai."', parent='".$parent."',idcity='".$tinh."',detail_short='".$txtDetailShort."',detail='".$detail."',type='".$loaihinh."' , price='".$price."',donvi='".$donvi."', dientich='".$dientich."', huong='".$huong."', tinhtrangphaply='".$phaply."', dtkhuonvien='".$dtkhuonvien."', solau='".$solau."', sophong='".$sophong."', namxaydung='".$namxaydung."', dacdiemkhac='".$ddk."', raoban_thue='".$raoban_thue."',  sort='".$sort."',  title='".$title."',  description='".$description."',  keyword='".$keyword."', last_modified=now() where id='".$oldid."'";
		}else{
			  $sql = "insert into tbl_rv_item (code, iduser ,cate , idcity , type , name, khachhang , diachi , email , dienthoai  ,   parent, detail_short, detail , sort, price , donvi , dientich , huong , tinhtrangphaply , dtkhuonvien , solau , sophong , namxaydung , dacdiemkhac , raoban_thue , title, description , keyword, status , date_added, last_modified, lang) values ('".$code."','".$iduser."',4,'".$tinh."','".$loaihinh."','".$name."','".$khachhang."','".$diachi."','".$email."','".$dienthoai."','".$parent."','".$txtDetailShort."','".$detail."','".$sort."','".$price."','".$donvi."','".$dientich."','".$huong."','".$phaply."','".$dtkhuonvien."','".$solau."','".$sophong."','".$namxaydung."','".$ddk."','".$raoban_thue."','".$title."','".$description."','".$keyword."','1',now(),now(),'".$lang."')";
		}
	...
	if ($errMsg == '')
		 echo '<script>window.location="admin.php?act=bds_info&cat='.$_REQUEST['cat'].'&page='.$_REQUEST['page'].'&code=1"</script>' ;
}else{
	if (isset($_GET['id'])){
		$oldid=$_GET['id'];
		$page = $_GET['page'];
		$sql = "select * from tbl_rv_item where id='".$oldid."'";
        ...
        <div class="box-widget">
            ...
            <div class="widget-container">
                <div class="widget-block">
                   ...
                   <form method="post" name="frmForm" enctype="multipart/form-data" action="admin.php?act=bds_info_m">



            

            <input type="hidden" name="act" value="bds_info_m">
                    ...
                    <tr>
                      ...
                      <script type="text/javascript">
...

                            {
								...
								filebrowserImageBrowseUrl : '../lib/ckfinderbds/ckfinder.html?Type=Images',

								filebrowserFlashBrowseUrl : '../lib/ckfinderbds/ckfinder.html?Type=Flash',

								filebrowserImageUploadUrl : '../lib/ckfinderbds/core/connector/php/connector.php?command=QuickUpload&type=Images',

								filebrowserFlashUploadUrl : '../lib/ckfinderbds/core/connector/php/connector.php?command=QuickUpload&type=Flash',									
