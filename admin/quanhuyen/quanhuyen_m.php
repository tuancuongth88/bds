...
if(isset($frame)==true){
	if($_SESSION['kt_login_level']!=4 && $_SESSION['kt_login_level']!=-1){
		...
		elseif(userPermissEdit($row_tbl_users['listEdit'],14,2)!=true) header("location: admin.php");
	}
}else{
...
if (isset($_POST['btnSave'])){
	...
	if($parent1==-1 || $parent1==""  ) $parent1=46;
	...
	$catInfo       = getRecord('tbl_quanhuyen', 'id='.$parent);
	if(!$multiLanguage){
		$lang      = $catInfo['lang'];
	}else{
	...
	if ($errMsg==''){
		if (!empty($_POST['id'])){
			$oldid = $_POST['id'];
			$sql = "update tbl_quanhuyen set name='".$name."' , parent='".$parent."' , sort='".$sort."' , last_modified=now()  where id='".$oldid."'";
		}else{
			$sql = "insert into tbl_quanhuyen (  name ,  sort, parent , status,  date_added, last_modified ) values ('" .$name."','".$sort."','".$parent."','1',now(),now())";
		}
		if (mysql_query($sql,$conn)){
			if(empty($_POST['id'])) $oldid = mysql_insert_id();
			$r = getRecord("tbl_quanhuyen","id=".$oldid);

			$arrField = array(
			"subject"          => "'".cat_kytu_dacbiet($name,1,1,0,1,1)."'"
			);// ko them id vao cuoi cho dep 
			$result = update("tbl_quanhuyen",$arrField,"id=".$oldid);
			...
			if($sqlUpdateField!='')	{
				$sqlUpdate = "update tbl_quanhuyen set $sqlUpdateField where id='".$oldid."'";
	...
	if ($errMsg == '')
		echo '<script>window.location="admin.php?act=quanhuyen&cat='.$_REQUEST['cat'].'&page='.$_REQUEST['page'].'&code=1"</script>';
}else{
	if (isset($_GET['id'])){
		$oldid=$_GET['id'];
		$page = $_GET['page'];
		$sql = "select * from tbl_quanhuyen where id='".$oldid."'";
            ...
            <div class="widget-container">
                <div class="widget-block">
                   ...
                   <form method="post" name="frmForm" enctype="multipart/form-data" action="admin.php?act=quanhuyen_m">
                        ...
                        <input type="hidden" name="act" value="quanhuyen_m">
