...
if(isset($frame)==true){
	if($_SESSION['kt_login_level']!=4 && $_SESSION['kt_login_level']!=-1){
		...
		if(userPermissEdit($row_tbl_users['listEdit'],14,1)!=true) header("location: admin.php");
	}
}else{
...
<?php 
if(userPermissEdit($row_tbl_users['listEdit'],14,7 )==true  ||  $_SESSION['kt_login_level']==4 ||  $_SESSION['kt_login_level']==-1){
?>
	...
	obj = this;
		...
		$.ajax({
		   ...
		   data: 'id='+ id +'&table=tbl_quanhuyen' +'&type=1&cate=14',
...
<?php 
if(userPermissEdit($row_tbl_users['listEdit'],14,6)==true ||  $_SESSION['kt_login_level']==4 ||  $_SESSION['kt_login_level']==-1 ){
?>
	...
	obj = this;
		...
		$.ajax({
		   ...
		   data: 'id='+ id +'&table=tbl_quanhuyen' +'&type=2&cate=14',
    ...
    <div class="span12">
        <div class="box-widget">
            ...
            <div class="widget-container">
				...
				<?
				if(userPermissEdit($row_tbl_users['listEdit'],14,4)==true ||  $_SESSION['kt_login_level']==4  ||   $_SESSION['kt_login_level']==-1  ){
					switch ($_GET['action']){
						case 'del' :
							$id = $_GET['id'];
							$r = getRecord("tbl_quanhuyen","id=".$id);
							$resultParent = mysql_query("select id from tbl_phuongxa where parent='".$id."'",$conn);
							if (mysql_num_rows($resultParent) <= 0){
								@$result = mysql_query("delete from tbl_quanhuyen where id='".$id."'",$conn);
								if ($result){
                ...
                $MAXPAGE=1;
				$totalRows=countRecord("tbl_quanhuyen",$where);
				
                if ($_REQUEST['cat']!='') $where="parent=".$_REQUEST['cat']; ?>
                <form method="POST" action="#" name="frmForm" enctype="multipart/form-data">
                <input type="hidden" name="page" value="<?=$page?>">
                <input type="hidden" name="act" value="quanhuyen">
                <?
               // $pageindex = createPage(countRecord("tbl_quanhuyen",$where),"./?act=quanhuyen&cat=".$_REQUEST['cat']."&page=",$MAXPAGE,$page)?>
                ...
                <? if ($_REQUEST['code']==1) $errMsg = 'Cập nhật thành công.'?>
                    ...
                    <table width="100%"  class="admin_table">
                            ...
                            <thead>
                                ...
                                <tr align="center" >
                                  <td valign="middle" style="background-color:#F0F0F0; height:40px; padding-left:20px" colspan="10">  
                                  		<? //comboCategory('ddCat',getArrayCategory('tbl_quanhuyen'),'list_tim_loc',$parent,1)?>
                               ...
                               <tr>
                                 ...
                                 <? if(userPermissEdit($row_tbl_users['listEdit'],14,2)==true ||  $_SESSION['kt_login_level']==4 ||   $_SESSION['kt_login_level']==-1 ){?>
                                  <div><a href="admin.php?act=quanhuyen_m">
					    ...
					    $sql="select *,DATE_FORMAT(date_added,'%d/%m/%Y %h:%i') as dateAdd,DATE_FORMAT(last_modified,'%d/%m/%Y %h:%i') as dateModify from tbl_quanhuyen where parent<>0 and $where $sortby   limit ".($startRow).",".$pageSize;
						...
						?>
                            <tr>
                                ...
                                <td align="center">
                                <? if(userPermissEdit($row_tbl_users['listEdit'],14,3)==true ||  $_SESSION['kt_login_level']==4 ||   $_SESSION['kt_login_level']==-1 ){?>
                                    <a href="admin.php?act=quanhuyen_m&cat=<?=$_REQUEST['cat']?>&page=<?=$_REQUEST['page']?>&id=<?=$row['id']?>"><img src="images/icon3.png"/></a>
                                 <? } ?>
                                 <? if(userPermissEdit($row_tbl_users['listEdit'],14,4)==true ||  $_SESSION['kt_login_level']==4 ||   $_SESSION['kt_login_level']==-1 ){?>
                                    <a  title="Xóa" href="admin.php?act=quanhuyen&action=del&page=<?=$_REQUEST['page']?>&id=<?=$row['id']?>" onclick="return confirm('Bạn có muốn xoá luôn không ?');" ><img src="images/icon4.png" width="20" border="0" /></a>
                                 <? } ?>
                                </td>
                            </tr>
                         <?php }?> 
