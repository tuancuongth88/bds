...
if(isset($frame)==true){
	if($_SESSION['kt_login_level']!=4 && $_SESSION['kt_login_level']!=-1){
		...
		if( isset($_GET['id']) || !empty($_POST['id'])) {if(userPermissEdit($row_tbl_users['listEdit'],13,3)!=true) header("location: admin.php");}
		elseif(userPermissEdit($row_tbl_users['listEdit'],13,2)!=true) header("location: admin.php");
	}
}else{
...
if (isset($_POST['btnSave'])){
	...
	if ($errMsg==''){
		...
		if (mysql_query($sql,$conn)){
			...
			$result = update("tbl_quanhuyen_category",$arrField,"id=".$oldid);
