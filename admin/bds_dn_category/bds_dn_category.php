...
if(isset($frame)==true){
	if($_SESSION['kt_login_level']!=4 && $_SESSION['kt_login_level']!=-1){
		...
		if(userPermissEdit($row_tbl_users['listEdit'],7,1)!=true) header("location: admin.php");
	}
}else{
...
if (isset($_POST['tim'])==true)//isset kiem tra submit
{
	...
	$_SESSION['kt_tukhoa_raovatcate']=$tukhoa;
...
$(document).ready(function() {	  
	...
	<?php 
if(userPermissEdit($row_tbl_users['listEdit'],7,7 )==true  ||  $_SESSION['kt_login_level']==4 ||  $_SESSION['kt_login_level']==-1){
?>
	...
	obj = this;
		...
		$.ajax({
		   ...
		   data: 'id='+ id +'&table=tbl_rv_category' +'&type=1&cate=7',
...
<?php 
if(userPermissEdit($row_tbl_users['listEdit'],7,6)==true ||  $_SESSION['kt_login_level']==4 ||  $_SESSION['kt_login_level']==-1 ){
?>
	...
	obj = this;
		...
		$.ajax({
		   ...
		   data: 'id='+ id +'&table=tbl_rv_category' +'&type=2&cate=7',
    ...
    <div class="span12">
        <div class="box-widget">
            ...
            <div class="widget-container">
				...
				<?
				if(userPermissEdit($row_tbl_users['listEdit'],7,4)==true ||  $_SESSION['kt_login_level']==4  ||   $_SESSION['kt_login_level']==-1  ){
					switch ($_GET['action']){
						case 'del' :
							$id = $_GET['id'];
							$r = getRecord("tbl_rv_category","id=".$id);
							$resultParent = mysql_query("select id from tbl_rv_category where parent='".$id."'",$conn);
							if (mysql_num_rows($resultParent) <= 0){
								@$result = mysql_query("delete from tbl_rv_category where id='".$id."'",$conn);
								if ($result){
                ...
                if($parent!=-1 || $parent1!=-1) {
						...
						$where="1=1 and cate=3  and (id='{$tukhoa}' or name LIKE '%$tukhoa%' or '{$tukhoa}'=-1) and  ( parent in ({$parenstrt}) or id=$parent)";
					}
                    else $where="1=1  and cate=3 and (id='{$tukhoa}' or name LIKE '%$tukhoa%' or '{$tukhoa}'=-1)";
					
					$where.=" AND ( status='{$anhien}' or '{$anhien}'=-1)  AND ( hot='{$noibat}' or '{$noibat}'=-1)";
				
                $MAXPAGE=1;
				$totalRows=countRecord("tbl_rv_category",$where);
				
                if ($_REQUEST['cat']!='') $where="parent=".$_REQUEST['cat']; ?>
                <form method="POST" action="#" name="frmForm" enctype="multipart/form-data">
                <input type="hidden" name="page" value="<?=$page?>">
                <input type="hidden" name="act" value="bds_dn_category">
                <?
               // $pageindex = createPage(countRecord("tbl_rv_category",$where),"./?act=bds_dn_category&cat=".$_REQUEST['cat']."&page=",$MAXPAGE,$page)?>
                ...
                <? if ($_REQUEST['code']==1) $errMsg = 'Cập nhật thành công.'?>
                    ...
                    <table width="100%"  class="admin_table">
                            ...
                            <thead>
                                ...
                                <tr align="center" >
                                  <td valign="middle"  colspan="10">
                                    <center>
                                        ...
                                        <strong>DANH MỤC DOANH NGHIỆP BẤT ĐỘNG SẢN</strong>
                                        </div>
                                    </center>
                                   </td>
                                </tr>
                                <tr align="center" >
                                  <td valign="middle" style="background-color:#F0F0F0; height:40px; padding-left:20px" colspan="10">  
                                  		<? //comboCategory('ddCat',getArrayCategory('tbl_rv_category'),'list_tim_loc',$parent,1)?>
                                ...
                                <tr >
                                  <td valign="middle" align="left" style="background-color:#F0F0F0; height:40px; padding-left:20px" colspan="10"> 
                                        <div class="link_loc" style="width:80px;text-align:center; padding:3px; border:solid 1px #999; float:left; margin-right:5px;<?php if($noibat==0  &&  $anhien==0) echo 'background-color:#FF0; color:#000;';else echo 'background-color:#FFF; color:#FFF;"';?>">
                                        	<a href="admin.php?act=bds_dn_category&tang=1&anhien=-1&noibat=-1">Tất cả</a>
                                        </div>
                                        <div class="link_loc" style="width:80px; text-align:center; padding:3px; border:solid 1px #999; float:left; margin-right:5px;<?php if($tang==1) echo 'background-color:#FF0; color:#000;';else echo 'background-color:#FFF; color:#FFF;"';?>" >
                                        	<a href="admin.php?act=bds_dn_category&tang=1&anhien=<?php echo $_SESSION['kt_anhien'] ?>&noibat=<?php echo $_SESSION['kt_noibat']?>">Tăng dần</a>
                                        </div>
                                        <div class="link_loc" style="width:80px;text-align:center; padding:3px; border:solid 1px #999; float:left; margin-right:5px;<?php if($tang==0) echo 'background-color:#FF0; color:#000;';else echo 'background-color:#FFF; color:#FFF;"';?>">
                                        	<a href="admin.php?act=bds_dn_category&tang=0&anhien=<?php echo $_SESSION['kt_anhien'] ?>&noibat=<?php echo $_SESSION['kt_noibat']?>">Giảm dần</a>
                                        </div>
                                        <div class="link_loc" style="width:80px;text-align:center; padding:3px; border:solid 1px #999; float:left; margin-right:5px;<?php if($anhien==1) echo 'background-color:#FF0; color:#000;';else echo 'background-color:#FFF; color:#FFF;"';?>">
                                        	<a href="admin.php?act=bds_dn_category&tang=<?php echo $_SESSION['kt_tang'] ?>&anhien=1&noibat=<?php echo $_SESSION['kt_noibat'] ?>"> Ẩn </a>
                                        </div>
                                         <div class="link_loc" style="width:80px;text-align:center; padding:3px; border:solid 1px #999; float:left; margin-right:5px;<?php if($anhien==0) echo 'background-color:#FF0; color:#000;';else echo 'background-color:#FFF; color:#FFF;"';?>">
                                        	<a href="admin.php?act=bds_dn_category&tang=<?php echo $_SESSION['kt_tang'] ?>&anhien=0&&noibat=<?php echo $_SESSION['kt_noibat'] ?>">Hiện</a>
                                        </div>
                                        <div class="link_loc" style="width:80px;text-align:center; padding:3px; border:solid 1px #999; float:left; margin-right:5px;<?php if($noibat==1) echo 'background-color:#FF0; color:#000;';else echo 'background-color:#FFF; color:#FFF;"';?>">
                                        	<a href="admin.php?act=bds_dn_category&tang=<?php echo $_SESSION['kt_tang'] ?>&anhien=<?php echo $_SESSION['kt_anhien'] ?>&noibat=1">Nổi bật</a>
                                        </div>
                                         <div class="link_loc" style="width:80px;text-align:center; padding:3px; border:solid 1px #999; float:left; margin-right:5px;<?php if($noibat==0) echo 'background-color:#FF0; color:#000;';else echo 'background-color:#FFF; color:#FFF;"';?>">
                                        	<a href="admin.php?act=bds_dn_category&tang=<?php echo $_SESSION['kt_tang'] ?>&anhien=<?php echo $_SESSION['kt_anhien'] ?>&noibat=0">ko nổi bật</a>
                                        </div>
                                  </td>
                                </tr>
                               <tr>
                                 ...
                                 <? if(userPermissEdit($row_tbl_users['listEdit'],7,2)==true ||  $_SESSION['kt_login_level']==4 ||   $_SESSION['kt_login_level']==-1 ){?>
                                  <div><a href="admin.php?act=bds_dn_category_m">
                                ...
                                <td align="center"><a class="title" href="<?=getLinkSort(4)?>">Thuộc danh mục</a></td>
					    ...
					    $sql="select *,DATE_FORMAT(date_added,'%d/%m/%Y %h:%i') as dateAdd,DATE_FORMAT(last_modified,'%d/%m/%Y %h:%i') as dateModify from tbl_rv_category where parent<>0 and $where $sortby   limit ".($startRow).",".$pageSize;
						$result=mysql_query($sql,$conn);
						$i=0;
						while($row=mysql_fetch_array($result)){
						$parent = getRecord('tbl_rv_category','id = '.$row['parent']);
						...
						?>
                            <tr>
                                ...
                                <td align="center">
                                 <? if(userPermissEdit($row_tbl_users['listEdit'],7,3)==true ||  $_SESSION['kt_login_level']==4 ||   $_SESSION['kt_login_level']==-1 ){?>
                                    <a href="admin.php?act=bds_dn_category_m&cat=<?=$_REQUEST['cat']?>&page=<?=$_REQUEST['page']?>&id=<?=$row['id']?>"><img src="images/icon3.png"/></a>
                                 <? } ?>
                                 <? if(userPermissEdit($row_tbl_users['listEdit'],7,4)==true ||  $_SESSION['kt_login_level']==4 ||   $_SESSION['kt_login_level']==-1 ){?>
                                    <a  title="Xóa" href="admin.php?act=bds_dn_category&action=del&page=<?=$_REQUEST['page']?>&id=<?=$row['id']?>" onclick="return confirm('Bạn có muốn xoá luôn không ?');" ><img src="images/icon4.png" width="20" border="0" /></a>
                                 <?php }?>
                                </td>
                            </tr>
                         <?php }?> 
