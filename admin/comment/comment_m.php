...
if(isset($frame)==true){
	if($_SESSION['kt_login_level']!=4 && $_SESSION['kt_login_level']!=-1){
		...
		if( isset($_GET['id']) || !empty($_POST['id'])) {if(userPermissEdit($row_tbl_users['listEdit'],10,3)!=true) header("location: admin.php");}
		elseif(userPermissEdit($row_tbl_users['listEdit'],10,2)!=true) header("location: admin.php");
	}
}else{
...
$(document).ready(function() {	
	...
	obj = this;
		...
		$.ajax({
		   ...
		   data: 'id='+ id +'&table=tbl_comment' +'&type=3',
                    ...
                    <table width="100%" class="admin_table">
                        ...
                        <tbody>
                          ...
                          <?
                            ...
                            $query = mysql_query("select * from tbl_comment where parent='".$_REQUEST['id']."' and top=0 order by id DESC");
                            ...
                            <tr>
                                ...
                                <td align="center"><a  title="Xóa" href="admin.php?act=comment_m&action=del&page=<?=$_REQUEST['page']?>&id=<?=$row['id']?>" onclick="return confirm('Bạn có muốn xoá luôn không ?');" ><img src="images/icon4.png" width="20" border="0" /></a>
							...
							<?php  
                                  $query1 = mysql_query("select * from tbl_comment where parent='".$row['id']."'   and top=1 order by id DESC"); 
                            ...
                            <tr>
                                ...
                                <td colspan="2" align="left"><div style="padding:5px; padding-left:20px;"><a href="http://batdongsan.jbs.vn/<?=get_field('tbl_rv_item','id',$row1['parent'],'subject');?>.html" target="_blank"><?=$row1['content']?> </a></div></td>
                                <td align="left">Người đăng: <?php $parent = getRecord('tbl_customer','id = '.$row1['iduser']);  if($row1['iduser']==1) echo "admin";elseif($row['iduser']==0) echo "Vãng lai";else echo $parent['username'];?> 
                                </td>
                                ...
                                <td align="center"><span class="smallfont"><img src="images/anhien_<?=$row1['status']?>.png" width="25" height="25" class="anhien" title="Ẩn hiện" value="<?=$row1['id']?>" /></span></td>
                              ...
                              <td align="center">
                                        ...
                                        <a  title="Xóa" href="admin.php?act=comment&action=del&page=<?=$_REQUEST['page']?>&id=<?=$row1['id']?>" onclick="return confirm('Bạn có muốn xoá luôn không ?');" ><img src="images/icon4.png" width="20" border="0" /></a> 
